...
def test_hapus_transaksi_retur_stok_akurat(db):
    ...
    for item in items:
        db.execute(
            text("INSERT INTO produk_batch (produk_id, qty_masuk, qty_sisa, harga_beli, tanggal_masuk) VALUES (:pid, :qty, :qty, :hb, CURRENT_TIMESTAMP)"),
...
def test_hapus_transaksi_multi_item_semua_diretur(db):
    ...
    for item in items:
        db.execute(text("INSERT INTO produk_batch (produk_id, qty_masuk, qty_sisa, harga_beli, tanggal_masuk) VALUES (:pid, :qty, :qty, :hb, CURRENT_TIMESTAMP)"),
                   {"pid": item.produk_id, "qty": item.qty, "hb": float(item.harga_beli)})
    db.execute(text("DELETE FROM transaksi_detail WHERE transaksi_id = :tid"), {"tid": trx_id})
    db.execute(text("DELETE FROM transaksi WHERE id = :id"), {"id": trx_id})
    db.commit()
...
def test_recalculate_setelah_edit_qty(db):
    """Total & profit header harus akurat setelah edit qty item, termasuk jika ada harga_tinta."""
    from app.transaksi.edit_item import recalculate_total_profit

    db.execute(text("INSERT INTO produk (id, kode, nama, harga_beli, harga_jual) VALUES (1,'P01','Prod A',1000,2000)"))
    db.execute(text("INSERT INTO produk_batch (produk_id, qty_masuk, qty_sisa, harga_beli, tanggal_masuk) VALUES (1, 100, 100, 1000, '2023-01-01')"))
    trx_id = buat_transaksi(db, produk_id=1, qty=5, hpp=1000, harga_jual=2000, total=10000, profit=5000)

    # Update qty dan kasih harga tinta untuk menguji formula fix
    db.execute(text("UPDATE transaksi_detail SET qty = 8, harga_tinta = 300 WHERE transaksi_id = :tid"), {"tid": trx_id})
    ...
    # Total omzet tidak boleh termasuk tinta = 8 * 2000 = 16000
    # Profit harus dikurangi tinta = (2000 - 1000 - 300) * 8 = 700 * 8 = 5600
    assert float(trx.total) == 16000.0
    assert float(trx.profit) == 5600.0
